    ...
    <header className="sticky top-0 z-50 border-b border-[#d9e4f1] bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-[4.75rem] max-w-7xl items-center justify-between px-5 lg:px-8">
        ...
        <nav aria-label="Primary navigation" className="hidden items-center gap-7 md:flex">
          {navigation.map(([label, href]) => (
            <a
              className="text-sm font-bold text-[#31415f] hover:text-[#0758ff]"
              href={href}
              key={href}
            >
              {label}
        ...
        <button
          ...
          className="rounded-sm border border-[#cbd9e8] p-2 text-[#09256f] md:hidden"
      ...
      {open ? (
        <nav
          ...
          className="border-t border-[#e3ebf4] bg-white px-5 py-4 md:hidden"
        >
          {navigation.map(([label, href]) => (
            <a
